...
class Help extends patron.Command {
  ...
  async run(msg, args) {
    if (String.isNullOrWhiteSpace(args.command)) {
      await msg.author.DM(
        'ModerationBot is **THE** cleanest bot around, and you can have it in **YOUR** server simply by clicking here: ' + Constants.data.links.botInvite + '.\n\nFor all information about command usage and setup on your Discord Sever, view the official documentation: ' + Constants.data.links.documentation + '.\n\nThe `' + Constants.data.misc.prefix + 'help <command>` command may be used for view the usage and an example of any command.\n\nThe `' + Constants.data.misc.prefix + 'modules [module]` command may be used for viewing all modules, or the information on a module.\n\nIf you have **ANY** questions, you may join the **Official ModerationBot Discord Server:** ' + Constants.data.links.serverInvite + ' for instant support along with a great community.');
